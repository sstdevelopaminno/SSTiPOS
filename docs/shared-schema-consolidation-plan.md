...
- Existing migration `202605190001_table_management_floor_plan.sql` already backfilled from `dine_in_tables` into `dining_tables` and moved `orders.table_id` to `dining_tables`.
...
Change added:
- Migration `supabase/migrations/202607180004_dine_in_tables_compat.sql` creates `public.dine_in_tables_compat` with `security_invoker = true`.
...
## POS runtime consolidation follow-up - 2026-07-18

Status: Migration hardening and web runtime retry guard completed.

Changes added:
- `next_pos_order_no` now has explicit function comments and restricted execute grants.
- POS web order creation retries unique `orders(tenant_id, branch_id, order_no)` conflicts by requesting a fresh shared order number.
- Direct fallback order creation uses the same shared order-number RPC and retries duplicate order numbers before returning an insert failure.
- Stock deduction, mobile checkout, and cleanup security-definer functions now have explicit comments and execute grants.
- Stock realtime publication migration now no-ops safely when `supabase_realtime` is not present.

Guardrail:
- Keep the unique constraint on `orders(tenant_id, branch_id, order_no)`.
- Keep retry handling in app code unless `next_pos_order_no` is moved fully inside every SQL insert transaction path.
