...
comment on table public.branch_inventory_settings is
  'Branch-level inventory runtime settings for POS stock deduction behavior.';

comment on function public.deduct_order_recipe_stock(uuid, uuid, uuid, text, uuid, text, text) is
  'Deducts recipe ingredient stock for an order and writes stock movements. Replaced by 202607180005 to make request_id unique per ingredient.';
