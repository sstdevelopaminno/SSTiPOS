...
comment on function public.next_pos_order_no(uuid, uuid, text) is
  'Returns the next short POS order number for a tenant/branch/prefix/day. Callers must still retry on orders(tenant_id, branch_id, order_no) unique conflicts because this RPC may be called outside the final insert transaction.';

revoke all on function public.next_pos_order_no(uuid, uuid, text) from public;
grant execute on function public.next_pos_order_no(uuid, uuid, text) to authenticated, service_role;
