...
function LoginDevicesPageContent() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const { response, body } = await fetchJsonWithRetry<DevicesResponse>("/api/auth/devices");
  ...
  return (
    <PreEntryShell mode={flow} activeStep={flow === "multi" ? 4 : 3} title="" subtitle="" layout="store" showModePill={false} showStepbar={false}>
      ...
      <div className="ipos-inline-actions ipos-branch-actions">
        <button type="button" className="ipos-outline-btn" onClick={() => router.push(`/login/employee?flow=${flow}`)} disabled={submitting}>
          ย้อนกลับ
        </button>
